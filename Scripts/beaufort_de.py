...
def main():
    ...
    root = ctk.CTk()
    root.title("Beaufort Cipher Solver")
        ...
        if state["isKey"]:
            ...
            def decryptvigenere(ciphertext, key):
                ...
                for i in range(len(list_ciphertext)):
                    #cipherindex = ciphertext.index(letters)
                    keyindex = key.index(key)
                    while not i >= len(list_key): #iterates until it reaches length of key
                        list_ciphertext[i] = list_key[i]
                        i += 1
                    else:
                        list_ciphertext[i] = list_ciphertext[i - len(list_key)] # and then it repeats!!!
                        i += 1

                joined = "".join(list_ciphertext)

                for i in range(len(ciphertext)):

                    #ciphertextindex = alphabet.index(joined[index]) #seems to assign numbers to letters 
                    #keyindex = alphabet.index(key[index % len(key)])

                    c = alphabet.index(ciphertext[i])
                    k = alphabet.index(list(joined)[i])
                    plaintext += alphabet[(k - c) % 26] #this is what makes it different from vigenere - its key - ciphertext
                #print("PASS")
                #print(list_ciphertext)
                #print("".join(list_ciphertext))
                #print(plaintext)
                textbox_plain.configure(state="normal")
                textbox_plain.delete("1.0", ctk.END)
                textbox_plain.insert("1.0", f"{plaintext}")
            ...
            KEYLENGTH = int(textbox_keylength.get("1.0", ctk.END).strip()) #like orange lol
            ...
            for keywordIndex in range(len(commonwords)):
                ...
                for let in cipher:
                    if let.isalpha():
                        ...
                        value = (keyValue - letValue + 26) % 26 + ord( 'a') # Not sure how, but I guessed this and it was right
                        plaintext += chr(value)
                        keyIndex += 1
            ...
            for keyword in commonwords:
                ...
                for let in cipher:
                    if let.isalpha():
                        letValue = ord(let) - ord('a')
                        keyValue = ord(keyword[keyIndex % len(keyword)]) - ord('a')
                        value = (keyValue - letValue + 26) % 26 + ord('a')
                        plaintext += chr(value)
                        keyIndex += 1
                    else:
                        plaintext += let
    ...
    label_keylength = ctk.CTkLabel(textbox_frame, text="Key Length:", font=ctk.CTkFont(size=16, weight="bold"))
    textbox_keylength = ctk.CTkTextbox(textbox_frame, font=("Courier New", 12), activate_scrollbars=False, height=30, width=200)
    ...
    if CONFIG.getboolean("SETTINGS", "QUICK_ENTER", fallback=False):
        textbox_keylength.bind("<Return>", lambda event: on_enter())

    def setKey():
        if switch_key_var.get() == "on":
            state["isKey"] = True
            textbox_key.grid(row=3, column=1, pady=10)
            label_keylength.grid_forget()
            textbox_keylength.grid_forget()
            #print("Key input enabled")
        else:
            state["isKey"] = False
            textbox_key.grid_forget()
            label_keylength.grid(row=3, column=0, pady=5)
            textbox_keylength.grid(row=4, column=0, pady=5)
