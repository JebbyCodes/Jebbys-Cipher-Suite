""""
thanks to this stack overflow person:
...
def main():
    ...
    def decipher():
        ...
        string = textbox_cipher.get("1.0", ctk.END).strip().upper()
        iterations = int(textbox_bruteforce.get("1.0", ctk.END).strip())
        list1 = top_n_list('en', 999999999)
        ...
        for a in range(iterations):
            for b in range(iterations):
                ...
                if any(word in decrypted for word in commonwords) and count > 0:
                    ...
                    textbox_plain.insert("1.0", f"{a},{b} -> {decrypted}, {count}" + "\n")
                    textbox_plain.configure(state="disabled")

            state["decipher_running"] = False

        #threading.Thread(target=run_cipher).start()
    ...
    btn_cipher_process = ctk.CTkButton(
        textbox_frame,
        text="Process Affine Cipher",
        command=lambda: start_decipher() if textbox_bruteforce.get("1.0", ctk.END).strip().isdigit() else (textbox_processing.configure(state="normal"), textbox_processing.insert("1.0", "ERROR: NON-INTEGER VALUE\n"), textbox_processing.configure(state="disabled"))
    )
    btn_cipher_process.grid(row=2, column=1, pady=10, columnspan=2)
...
# -- General Processing -- #
    ...
    textbox_processing.grid(row=4, columnspan=2, padx=10, sticky="nsew")
    ...
    label_bruteforce = ctk.CTkLabel(textbox_frame, text="How hard do you want to bruteforce your text?", font=ctk.CTkFont(size=14))
